  ...
  return (
    <div>
      ...
      <Card glow={C.accent}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 20, marginBottom: 16 }}>
          ...
          <div>
            ...
            <Chip label="Good Standing ↑4 this month" color={C.green} />
          </div>
        </div>
      ...
      <Card glow={faithMode ? C.gold : undefined}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <div>
              <div style={{ fontSize: 14, fontWeight: 700, color: C.text }}>Faith Mode</div>
              <div style={{ fontSize: 11, color: C.sub }}>Biblical stewardship wisdom on dashboard</div>
            </div>
          </div>
          <div
            onClick={() => setFaithMode(f => !f)}
            style={{ width: 48, height: 26, borderRadius: 99, cursor: "pointer",
              background: faithMode ? C.gold : C.border, transition: "background .2s", position: "relative" }}
          >
            <div style={{ position: "absolute", top: 3, left: faithMode ? 25 : 3,
      ...
      <Card>
        ...
        {[
          { label: "Food delivery overspend", amount: 412 },
          { label: "Unused subscriptions",    amount: 168 },
          { label: "Impulse purchases",        amount:  94 },
        ].map(l => (
          <div key={l.label} style={{ display: "flex", justifyContent: "space-between",
            ...
            <span style={{ color: C.rose, fontWeight: 700 }}>${l.amount}</span>
          </div>
        ))}
